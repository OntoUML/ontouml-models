...
The workflows use Python **3.11**, but local use is not restricted to that version. The pinned converter declares Python `>=3.10,<4.0`, which includes **3.13**. Use an installed compatible interpreter and verify the resolved dependencies and repository tests below; the declared range alone does not guarantee that every environment works. Install the existing [requirements](requirements.txt); the ontology wrapper requires exactly `ontouml-json2graph==2.0.2` and rejects another converter version. The repository's date-based release tags do not version the converter, schema, or vocabularies together.
