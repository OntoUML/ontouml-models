...
- `vocabulary.ttl`: the Turtle file that contains the OntoUML Metadata Vocabulary used in the description of the catalog resources.
...
- `ontology.ttl`: the Turtle serialization of the model in linked data format described with the [OntoUML Vocabulary](https://w3id.org/ontouml/vocabulary). This file is automatically generated from the JSON serialization.
...
As we could not satisfy the metadata needs of our stakeholders using the existing vocabularies alone, we complemented them with one of our own—the [OntoUML/UFO Catalog Metadata Vocabulary](https://w3id.org/ontouml-models/vocabulary). The elements defined there are identified below by the prefix **ocmv**.
...
- FDP Catalog item page: https://w3id.org/ontouml-models
